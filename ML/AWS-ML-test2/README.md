...
```bash
AWS-ML-test2$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
AWS-ML-test2$ sam local start-api
AWS-ML-test2$ curl http://localhost:3000/classify_digit
```
...
```bash
AWS-ML-test2$ sam logs -n InferenceFunction --stack-name "aws-ml-test2" --tail
```
...
```bash
sam delete --stack-name "aws-ml-test2"
```
